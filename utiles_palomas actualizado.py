...
#Esta es la parte opcional , es para guardar el archivo

def guardar_datos(palomas, archivo="palomas.txt"):
    with open(archivo, "w", encoding="utf-8") as f:
        for anilla in palomas:
            p = palomas[anilla]
            linea = f"{anilla};{p['nombre']};{p['propietario']};{p['tiempo']};{p['velocidad']}\n"
            f.write(linea)
    return True

def cargar_datos(palomas, archivo="palomas.txt"):
    try:
        with open(archivo, "r", encoding="utf-8") as f:
            for linea in f:
                ...
                palomas[anilla] = {"nombre": nombre, "propietario": propietario, "tiempo": tiempo, "velocidad": velocidad}
        return True
